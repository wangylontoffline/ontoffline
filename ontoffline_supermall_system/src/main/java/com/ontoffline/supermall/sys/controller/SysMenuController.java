...
import com.ontoffline.supermall.sys.constant.MenuType;
...
//	}
	...
	private void verifyForm(SysMenu menu){
		...
		if(menu.getType() == MenuType.MENU.getValue()){
			if(StrUtil.isBlank(menu.getUrl())){
				throw new OntofflineSupermallBindException("菜单URL不能为空");
			}
		}
		if(Objects.equals(menu.getMenuId(), menu.getParentId())){
			throw new OntofflineSupermallBindException("自己不能是自己的上级");
		}
		...
		if(menu.getType() == MenuType.CATALOG.getValue() ||
			...
			if(parentType != MenuType.CATALOG.getValue()){
				throw new OntofflineSupermallBindException("上级菜单只能为目录类型");
			}
			return ;
		}
		...
		if(menu.getType() == MenuType.BUTTON.getValue()){
			if(parentType != MenuType.MENU.getValue()){
				throw new OntofflineSupermallBindException("上级菜单只能为菜单类型");
			}
		}
	}
}
